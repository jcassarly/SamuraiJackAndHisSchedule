...
\hline
account & AccountManager & The object that handles the account, specifically when the Hamburger Menu is open. \\
\hline
events & Event[] & The list of events being displayed on the calendar \\
\hline
calendarComponent & UIObject & The UI view of the calendar that is displayed to the user \\
\hline
toolbarComponent & Toolbar & The object that stores the state of the Toolbar for the calendar \\
\hline
date & Date Object & the date being displayed on the Home Screen
\hline
dateComponent & TextBox & The UI object that displays the date at the top of the screen \\
\hline
hamburgerMenuState & Boolean & true if the Hamburger Menu is open, false if closed \\
\hline
hamburgerMenuButton & Button & The button the user can click to open the Hamburger Menu \\
\hline
...
\textbf{Attribute Name} & \textbf{Type} & \textbf{Description} \\
\hline
inputType & FieldEnum & Enum object that specifies the choices for the type of input field to load: STANDARD, LOCATION, DEADLINE, SETTINGS, and EVENT.  EVENT specifies a drop-down menu to load for choosing a different enum value. \\
\hline
\end{longtable}
\end{center}
...
\hline
currentTool & ToolEnum & void & Toolbar indicates which tool is currently selected on it.\\
\hline
notifyUIEvent & void & UIEvent & This function is called internally to notify any objects subscribed to the toolbar of an event that occurred. This will generally be used when the user clicks on a button.\\
\hline
\end{longtable}
\end{center}
...
\textbf{Field Name} & \textbf{Field Type} & \textbf{Field Description} \\
\hline
frequencyType & FreqEnum & An enum with the values DAILY, WEEKLY, MONTHLY, YEARLY, and CUSTOM. \\
\hline
...
\textbf{Method Name} & \textbf{Parameters} & \textbf{Method Output Type} & \textbf{Method Description} \\
\hline
getFrequency & void & FreqEnum & This returns frequency \\
\hline
...
\textbf{Field Name} & \textbf{Field Type} & \textbf{Field Description} \\
\hline
notificationType & NotificationEnum & An enum with the values EMAIL, TEXT, PUSH, and BANNER \\
\hline
...
\textbf{Method Name} & \textbf{Parameters} & \textbf{Method Output Type} & \textbf{Method Description} \\
\hline
getNotificationType & void & NotificationEnum & This returns notificationType \\
\hline
