...
\\\\
One exception to the aforementioned exiting process is with Deadline Events.  Once the user clicks save on a Deadline event, they will be presented with a diff between their old calendar and the changes the auto scheduler is proposing based on the information provided for the Deadline Event.  The user will be given the option to accept the changes or reject them (returning them to the input form).  Upon acceptance, the changes are saved and the user goes back to the home screen.
\\\\
...
\\\\
The information on this page of the application will include all items specified in the General Settings Requirements.  The following table describes the Input Fields in more detail:
